...
export default function CardEvolucao({ columns = [], rows = [] }) {
  ...
  return (
    <div className="rounded-3xl bg-slate-800/70 border border-white/10 shadow-lg p-4 w-[590px] h-[320px] overflow-hidden flex flex-col">
      ...
      {/* GRÁFICO RESPONSIVO */}
      <div className="flex-1 min-h-0 rounded-xl border border-white/10 bg-slate-900/80 p-3 overflow-x-auto overflow-y-hidden">
        <div className="flex items-end gap-1 min-w-max h-full">
          {totals.map((valor, i) => {
            const altura = animate ? Math.max(4, Math.round((valor / max) * 100)) : 4;
            const [mes, ano] = normalizedColumns[i].split("/");

            return (
              <div key={i} className="flex flex-col items-center gap-1 w-10">
                <div
                  className="w-full rounded-md bg-sky-400/80 hover:bg-sky-300 transition-all duration-700 ease-out"
                  style={{ height: `${altura}%` }}
                  onMouseEnter={(e) => {
