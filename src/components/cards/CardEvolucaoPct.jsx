          ...
          <table className="border-separate border-spacing-0 w-full">
            ...
            <tbody>
              {pctRows.map((row, idx) => {
                ...
                return (
                  <tr key={row.ativo}
                    className={`${zebra ? "bg-white/[0.02]" : "bg-transparent"} hover:bg-white/[0.04]`}>
                    ...
                    {row.pcts.map((v, i) => {
                      if (i === 0)
                        return (
                          <td key={i}
                            className="px-3 py-2 border-b border-white/10 text-slate-500">
                            —
                          </td>
                        );
                      ...
                      return (
                        <td key={i}
                          className={`px-3 py-2 border-b border-white/10 tabular-nums ${
                            v === null
                              ? "text-slate-500"
                              : isUp
                              ? "text-emerald-400"
                ...
                <tr className="bg-slate-800/90 backdrop-blur text-sm">
                  ...
                  {totalPct.map((v, i) => {
                    if (i === 0)
                      return (
                        <td key={i}
                          className="px-3 py-2 border-t border-white/10 text-slate-400">
                          —
                        </td>
                      );
                    ...
                    return (
                      <td key={i}
                        className={`px-3 py-2 border-t border-white/10 tabular-nums ${
                          v === null
                            ? "text-slate-400"
                            : isUp
                            ? "text-emerald-400"
