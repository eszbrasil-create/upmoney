// src/pages/InvistaExterior.jsx
// Página full-screen "Invista no Exterior", com layout escuro e cabeçalho no padrão da Landing
...
export default function InvistaExterior({ onNavigate }) {
  const WHATSAPP_NUMBER = "5585999999999";

  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
      {/* Header (mesmo padrão da Saída Fiscal) */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="rounded-xl bg-[#d6af5f] px-4 py-2 font-medium text-[#1f3548] hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-sky-400/60"
            >
              Meu Plano
            </button>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg:white/20 hover:bg-white/20"
              aria-label="YouTube"
            >
      ...
      <main className="flex-1">
        <section className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16 grid md:grid-cols-2 gap-10 items-center">
            {/* Coluna texto */}
            <div>
              <p className="text-sky-300 text-xs font-semibold tracking-[0.2em] uppercase mb-3">
                Investindo em Dólar & Mundo
              </p>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-50">
                Construa patrimônio em{" "}
                <span className="text-sky-300">moeda forte</span>, passo a
                passo
              </h1>

              <p className="mt-4 text-sm sm:text-base text-slate-300 max-w-xl">
                Te ajudo a entender como abrir conta em corretora no exterior,
                escolher ativos globais e montar uma estratégia em dólar
                conectada ao que você já tem no Brasil.
              </p>

              <ul className="mt-5 space-y-2 text-sm text-slate-300">
                <li>• Entenda na prática como funciona uma corretora global.</li>
                <li>• Ações, ETFs, REITs e renda fixa em moeda forte.</li>
                <li>• Como integrar o exterior ao seu plano de longo prazo.</li>
              </ul>

              <div className="mt-8 flex flex-wrap items-center gap-3">
                <a
                  href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
                    "Olá! Quero entender como investir no exterior com o projeto Meu Patrimônio."
                  )}`}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-xl bg-sky-500 px-5 py-3 font-semibold text-slate-950 shadow-lg shadow-sky-500/30 hover:bg-sky-400"
                >
                  <IconWhatsApp className="h-5 w-5" />
                  Quero investir no exterior
                </a>

                <button className="inline-flex items-center rounded-xl border border-slate-600 px-4 py-2.5 text-sm font-medium text-slate-200 hover:border-sky-400 hover:text-sky-300">
                  Ver passos do plano
                </button>
              </div>
            </div>

            {/* Coluna card plano em dólar */}
            <div className="md:pl-4">
              <div className="rounded-2xl border border-slate-700/80 bg-slate-900/70 p-5 shadow-2xl shadow-sky-500/15">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-sm font-semibold text-slate-100">
                      Plano em Dólar — Visão Geral
                    </h2>
                    <p className="text-xs text-slate-400">
                      Uma estrutura simples para começar com segurança
                    </p>
                  </div>
                  <span className="rounded-full bg-sky-500/10 px-3 py-1 text-[11px] font-semibold text-sky-300">
                    Etapas claras
                  </span>
                </div>
                ...
                <ol className="space-y-3 text-sm">
                  <li className="flex gap-3">
                    <div className="mt-0.5 h-6 w-6 rounded-full bg-sky-500/20 flex items-center justify-center text-[11px] font-bold text-sky-300">
                      1
                    </div>
                    <div>
                      <p className="font-semibold text-slate-100">
                        Base financeira no Brasil organizada
                      </p>
                      <p className="text-xs text-slate-400">
                        Primeiro entendemos seu momento atual, fluxo de caixa e
                        reserva de emergência para não “pular etapas”.
                      </p>
                    </div>
                  </li>
                  ...
                  <li className="flex gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-100">
                        Estruturação da conta no exterior
                      </p>
                      <p className="text-xs text-slate-400">
                        Escolha de corretora internacional, envio de recursos,
                        câmbio e primeiros aportes com clareza de custos.
                      </p>
                    </div>
                  </li>
                  ...
                  <li className="flex gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-100">
                        Montagem da carteira global
                      </p>
                      <p className="text-xs text-slate-400">
                        Definição entre ETFs, ações, REITs e renda fixa,
                        sempre conectando com seus objetivos de prazo e perfil.
                      </p>
                    </div>
                  </li>
                </ol>
                ...
                <div className="mt-5 rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2 text-[11px] text-slate-300">
                  <p>
                    ...
                    você não precisa começar com muito dinheiro — precisa de um
                    plano simples, frequente e sustentável em moeda forte.
                  </p>
                </div>
              </div>
            </div>
      ...
      <footer className="mt-10 bg-[#020617] text-white/95 border-t border-slate-800/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="mt-6 text-xs text-white/70">
            Meu Patrimônio — Educação e estratégias globais para construir
