...
import { ArrowRight, Sparkles, PiggyBank, Shield } from "lucide-react";
    ...
    >
      ...
      <div className="p-5">
        <div className="grid gap-2">
          {bullets.map((item) => (
            <div key={item} className="text-sm text-slate-200 flex gap-2">
...
export default function CursosMenu({ onNavigate }) {
  return (
    <div className="pt-3 pr-6 pl-0">
      {/* max-w-8xl -> max-w-7xl (tailwind padrão) */}
      <div className="mx-auto w-full max-w-7xl rounded-2xl bg-slate-800/70 border border-white/10 shadow-lg p-6">
        {/* HEADER */}
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <CourseCard
            ...
            onClick={() => onNavigate?.("curso-previdencia-privada")}
          />
        </div>
      </div>
    </div>
